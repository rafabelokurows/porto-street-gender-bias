...
unique(streets$osm_lines$name) %>% iconv("UTF-8","latin1") %>% sort() %>% as.data.frame() %>%
  rename(rua=1) %>%
...
ruas2
ruas3 = openxlsx::read.xlsx("C:\\Users\\belokurowsr\\Downloads\\ruas.xlsx",sheet = "Sheet6")
ruas3 %>% View()
...
#   mutate(prop=n/sum(n))
str(ruas4)
str(streets$osm_lines)
class(ruas4)

streets$osm_lines %>%
  mutate(name = iconv(name,"UTF-8","latin1")) %>%
  left_join(ruas4 %>%   select(rua,clean,sexoFinal2),by=c("name"="rua")) %>%
  select(name,clean,sexoFinal2) %>% distinct(name,clean,sexoFinal2) %>% filter(is.na(clean))

ruas %>% left_join(ruas4,by=c("rua","nome","clean"))

ruas %>% filter(!clean %in% ruas4$clean)
